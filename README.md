...
**agentbounds** is an adversarial stress-tester for LLM agents. Point it at any agent, throw hostile and edge-case inputs at it, and get an honest report of where and how often it breaks. Zero runtime dependencies, 170 passing tests. Three decisions define it: hold rates over N trials instead of single verdicts, inconclusive judge calls excluded from the denominator instead of silently counted as passes, and heuristic detectors labelled as heuristic. The effect-assertion layer is still being built, and the README does not claim otherwise.
